...
Memory subsystem public API and persistence contracts.

This api document for alani-memory provides implementation-ready guidance for Rust developers. Specify memory device and knowledge record APIs.
...
- Primary aspects: MemoryDevice trait, KnowledgeRecord, query API, snapshot API.
...
- ALANI-D32-REQ-01: alani-memory MUST build independently where possible and MUST document sibling repository requirements.
- ALANI-D32-REQ-02: alani-memory MUST expose only the APIs named in this document until an RFC expands them.
- ALANI-D32-REQ-03: Every public item MUST include rustdoc explaining safety, errors, and examples.
- ALANI-D32-REQ-04: Every repository MUST include README, Cargo.toml, src, tests, and CI workflow files.
...
- ALANI-D32-AC-01: A developer can create alani-memory from the provided template and understand its first implementation tasks.
- ALANI-D32-AC-02: README commands are copy-pasteable after repositories are placed as siblings.
- ALANI-D32-AC-03: Public APIs are traceable to an architecture or data-format document.
...
- RISK-D32-01: Repository drift can occur if shared interfaces are duplicated without tests.
- RISK-D32-02: Path dependencies in templates must be adjusted when repositories are published independently.
