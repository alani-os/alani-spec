# Document 40: alani-terminal: API
...
Terminal command API and scripting interface.

This api document for alani-terminal provides implementation-ready guidance for Rust developers. Specify command registration and shell scripting API.
...
- Primary aspects: Command trait, CommandContext, parser, output format.
...
- ALANI-D40-REQ-01: alani-terminal MUST build independently where possible and MUST document sibling repository requirements.
- ALANI-D40-REQ-02: alani-terminal MUST expose only the APIs named in this document until an RFC expands them.
- ALANI-D40-REQ-03: Every public item MUST include rustdoc explaining safety, errors, and examples.
- ALANI-D40-REQ-04: Every repository MUST include README, Cargo.toml, src, tests, and CI workflow files.
...
- ALANI-D40-AC-01: A developer can create alani-terminal from the provided template and understand its first implementation tasks.
- ALANI-D40-AC-02: README commands are copy-pasteable after repositories are placed as siblings.
- ALANI-D40-AC-03: Public APIs are traceable to an architecture or data-format document.
...
- RISK-D40-01: Repository drift can occur if shared interfaces are duplicated without tests.
- RISK-D40-02: Path dependencies in templates must be adjusted when repositories are published independently.
