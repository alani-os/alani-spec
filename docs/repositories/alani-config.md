# Repository Specification: alani-config
...
Typed configuration schemas for boot, devices, runtime, security, corpus, release, and environment
profiles.

## Boundary Definition

`alani-config` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/profiles.rs` - owns the profiles API and implementation seam.
- `src/validation.rs` - owns the validation API and implementation seam.
...
tier = "MVK required"
owner = "Platform and runtime teams"
architectural_dependencies = ["alani-protocol"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-config`.
...
- `alani-config` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
