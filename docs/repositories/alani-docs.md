# Repository Specification: alani-docs
...
Source-of-truth specification docs, diagrams, ADRs, generated API references, lints, and publication
pipeline.

## Boundary Definition

`alani-docs` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/adr.rs` - owns the adr API and implementation seam.
- `src/lint.rs` - owns the lint API and implementation seam.
- `src/render.rs` - owns the render API and implementation seam.
- `src/diagrams.rs` - owns the diagrams API and implementation seam.
...
tier = "MVK required"
owner = "Documentation team"
architectural_dependencies = ["alani-config"]
aliases = ["alani-spec"]
```
...
1. Instantiate the template from `repo-templates/individual/alani-docs`.
...
- `alani-docs` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
