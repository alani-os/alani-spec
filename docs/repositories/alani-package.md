# Repository Specification: alani-package
...
Package format, local registry, dependency resolver, installation transaction model, and package
integrity checks.

## Boundary Definition

`alani-package` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
| `alani-identity` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
...
- `src/package.rs` - owns the package API and implementation seam.
- `src/resolver.rs` - owns the resolver API and implementation seam.
- `src/install.rs` - owns the install API and implementation seam.
...
tier = "Post-MVK boundary"
owner = "Runtime and release teams"
architectural_dependencies = ["alani-abi", "alani-protocol", "alani-config", "alani-policy", "alani-filesystem", "alani-identity"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-package`.
...
- `alani-package` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
