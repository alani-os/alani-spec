# Repository Specification: alani-observability
...
|---|---|
| Project | Alani |
| Repository | `alani-observability` |
| Version | 0.2.0-draft |
...
Tracing, metrics, diagnostics, event schema, span context, redaction classification, and export
adapters.

## Boundary Definition

`alani-observability` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/trace.rs` - owns the trace API and implementation seam.
- `src/metrics.rs` - owns the metrics API and implementation seam.
- `src/diagnostics.rs` - owns the diagnostics API and implementation seam.
- `src/export.rs` - owns the export API and implementation seam.
...
tier = "MVK required"
owner = "Observability team"
architectural_dependencies = ["alani-abi", "alani-protocol"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-observability`.
...
- `alani-observability` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
