# Repository Specification: alani-security
...
Security primitives, authorization helpers, crypto provider traits, authentication plumbing, and
sandbox mechanism boundaries.

## Boundary Definition

`alani-security` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/auth.rs` - owns the auth API and implementation seam.
- `src/crypto.rs` - owns the crypto API and implementation seam.
- `src/capability.rs` - owns the capability API and implementation seam.
- `src/sandbox.rs` - owns the sandbox API and implementation seam.
...
tier = "MVK required"
owner = "Security team"
architectural_dependencies = ["alani-lib", "alani-abi", "alani-policy", "alani-identity"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-security`.
...
- `alani-security` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
