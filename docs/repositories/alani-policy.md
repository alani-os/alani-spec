# Repository Specification: alani-policy
...
Declarative capability, access-control, sandbox, audit, and resource policies separated from
enforcement mechanisms.

## Boundary Definition

`alani-policy` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/rules.rs` - owns the rules API and implementation seam.
- `src/evaluator.rs` - owns the evaluator API and implementation seam.
...
tier = "MVK required"
owner = "Security and policy teams"
architectural_dependencies = ["alani-abi", "alani-protocol", "alani-config"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-policy`.
...
- `alani-policy` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
