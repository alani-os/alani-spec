# Repository Specification: alani-cognition
...
Cognitive execution service for model loading, inference requests, planning, memory retrieval, and
accelerator integration.

## Boundary Definition

`alani-cognition` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
| `alani-models` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
...
- `src/engine.rs` - owns the engine API and implementation seam.
- `src/model.rs` - owns the model API and implementation seam.
- `src/planner.rs` - owns the planner API and implementation seam.
- `src/retrieval.rs` - owns the retrieval API and implementation seam.
...
tier = "MVK required"
owner = "Cognition team"
architectural_dependencies = ["alani-lib", "alani-protocol", "alani-memory", "alani-models", "alani-devices", "alani-security", "alani-observability"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-cognition`.
...
- `alani-cognition` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
