# Repository Specification: alani-filesystem
...
Virtual filesystem, path resolution, mount table, inode model, journaling hooks, and file
capabilities.

## Boundary Definition

`alani-filesystem` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/vfs.rs` - owns the vfs API and implementation seam.
- `src/inode.rs` - owns the inode API and implementation seam.
- `src/path.rs` - owns the path API and implementation seam.
- `src/mount.rs` - owns the mount API and implementation seam.
- `src/journal.rs` - owns the journal API and implementation seam.
...
tier = "MVK required"
owner = "Storage and filesystem team"
architectural_dependencies = ["alani-abi", "alani-protocol", "alani-storage", "alani-policy", "alani-audit", "alani-observability"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-filesystem`.
...
- `alani-filesystem` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
