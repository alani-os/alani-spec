# Repository Specification: alani-models
...
Model manifests, adapters, compatibility tests, quantization metadata, and model-pack interface
definitions.

## Boundary Definition

`alani-models` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/adapter.rs` - owns the adapter API and implementation seam.
- `src/quantization.rs` - owns the quantization API and implementation seam.
- `src/compat.rs` - owns the compat API and implementation seam.
...
tier = "Post-MVK boundary"
owner = "Cognition team"
architectural_dependencies = ["alani-protocol", "alani-cognition", "alani-corpus", "alani-policy"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-models`.
...
- `alani-models` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
