# Repository Specification: alani-sim
...
`alani-sim` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
| `alani-network` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
| `alani-observability` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
| `alani-boot` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
...
- `src/qemu.rs` - owns the qemu API and implementation seam.
- `src/fake_devices.rs` - owns the fake devices API and implementation seam.
- `src/determinism.rs` - owns the determinism API and implementation seam.
...
tier = "MVK required"
owner = "Simulation team"
architectural_dependencies = ["alani-platform", "alani-devices", "alani-storage", "alani-network", "alani-observability", "alani-boot"]
aliases = ["alani-emulator"]
```
...
1. Instantiate the template from `repo-templates/individual/alani-sim`.
...
- `alani-sim` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
