# Repository Specification: alani-benchmarks
...
Benchmark scenarios and reporting for scheduler, syscalls, memory, IPC, filesystem, cognition, and
devices.

## Boundary Definition

`alani-benchmarks` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
| `alani-ipc` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
| `alani-storage` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
| `alani-filesystem` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
| `alani-cognition` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
| `alani-memory` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
| `alani-sim` | Consume stable public APIs, schemas, generated artifacts, or release evidence only. |
...
- `src/scenarios.rs` - owns the scenarios API and implementation seam.
- `src/metrics.rs` - owns the metrics API and implementation seam.
- `src/reports.rs` - owns the reports API and implementation seam.
- `src/baselines.rs` - owns the baselines API and implementation seam.
...
tier = "MVK required"
owner = "Performance team"
architectural_dependencies = ["alani-abi", "alani-ipc", "alani-storage", "alani-filesystem", "alani-cognition", "alani-memory", "alani-sim"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-benchmarks`.
...
- `alani-benchmarks` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
