# Repository Specification: alani-lib
...
Shared safe wrappers, result types, error vocabulary, no_std utilities, and ergonomic APIs used by
userspace and services.

## Boundary Definition

`alani-lib` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/abi.rs` - owns the abi API and implementation seam.
- `src/error.rs` - owns the error API and implementation seam.
...
1. Instantiate the template from `repo-templates/individual/alani-lib`.
...
- `alani-lib` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
