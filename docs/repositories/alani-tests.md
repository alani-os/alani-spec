# Repository Specification: alani-tests
...
Cross-repository integration, ABI conformance, syscall compatibility, security regression, and
fixture tests.

## Boundary Definition

`alani-tests` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/conformance.rs` - owns the conformance API and implementation seam.
- `src/fixtures.rs` - owns the fixtures API and implementation seam.
- `src/harness.rs` - owns the harness API and implementation seam.
- `src/regression.rs` - owns the regression API and implementation seam.
...
tier = "MVK required"
owner = "QA team"
architectural_dependencies = ["alani-abi", "alani-protocol", "alani-ipc", "alani-filesystem", "alani-sim", "alani-corpus", "alani-policy"]
aliases = ["alani-conformance"]
```
...
1. Instantiate the template from `repo-templates/individual/alani-tests`.
...
- `alani-tests` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
