# Repository Specification: alani-runtime
...
Userspace runtime for process and agent lifecycle, manifest loading, syscall wrapping, and workload
supervision.

## Boundary Definition

`alani-runtime` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/agent.rs` - owns the agent API and implementation seam.
- `src/loader.rs` - owns the loader API and implementation seam.
- `src/process.rs` - owns the process API and implementation seam.
...
tier = "MVK required"
owner = "Runtime team"
architectural_dependencies = ["alani-lib", "alani-abi", "alani-protocol", "alani-ipc", "alani-config", "alani-security", "alani-audit", "alani-observability"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-runtime`.
...
- `alani-runtime` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
