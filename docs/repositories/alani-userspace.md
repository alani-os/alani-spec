# Repository Specification: alani-userspace
...
Example agents, user applications, and integration examples demonstrating runtime, syscall,
cognition, and library use.

## Boundary Definition

`alani-userspace` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/examples.rs` - owns the examples API and implementation seam.
- `src/agents.rs` - owns the agents API and implementation seam.
- `src/apps.rs` - owns the apps API and implementation seam.
...
tier = "MVK required"
owner = "Userspace team"
architectural_dependencies = ["alani-runtime", "alani-lib", "alani-cognition", "alani-protocol"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-userspace`.
...
- `alani-userspace` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
