# Repository Specification: alani-sdk
...
Developer CLI, repository templates, sysroot management, code generation, local build helpers, and
compatibility checks.

## Boundary Definition

`alani-sdk` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/cli.rs` - owns the cli API and implementation seam.
- `src/codegen.rs` - owns the codegen API and implementation seam.
- `src/templates.rs` - owns the templates API and implementation seam.
- `src/sysroot.rs` - owns the sysroot API and implementation seam.
...
tier = "MVK required"
owner = "Developer experience team"
architectural_dependencies = ["alani-config", "alani-protocol", "alani-docs"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-sdk`.
...
- `alani-sdk` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
