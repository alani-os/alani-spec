# Repository Specification: alani-memory
...
Knowledge, vector, cache, and persistent memory abstractions consumed by cognition and runtime
services.

## Boundary Definition

`alani-memory` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/knowledge.rs` - owns the knowledge API and implementation seam.
- `src/vector.rs` - owns the vector API and implementation seam.
- `src/cache.rs` - owns the cache API and implementation seam.
- `src/nvm.rs` - owns the nvm API and implementation seam.
...
tier = "MVK required"
owner = "Memory team"
architectural_dependencies = ["alani-lib", "alani-abi", "alani-storage", "alani-observability"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-memory`.
...
- `alani-memory` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
