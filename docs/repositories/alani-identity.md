# Repository Specification: alani-identity
...
Principals, users, service identities, credentials, local identity store, session model, and
attestation subjects.

## Boundary Definition

`alani-identity` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/principal.rs` - owns the principal API and implementation seam.
- `src/credential.rs` - owns the credential API and implementation seam.
- `src/session.rs` - owns the session API and implementation seam.
- `src/store.rs` - owns the store API and implementation seam.
...
tier = "MVK required"
owner = "Security team"
architectural_dependencies = ["alani-abi", "alani-protocol", "alani-security", "alani-audit"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-identity`.
...
- `alani-identity` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
