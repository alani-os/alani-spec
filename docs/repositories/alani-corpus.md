# Repository Specification: alani-corpus
...
Corpus schema, sample data, validators, generators, labeling taxonomy, split manifests, and
datasheet tooling.

## Boundary Definition

`alani-corpus` is a top-level repository. It owns its own Cargo manifest, CI workflow, README,
...
- `src/validator.rs` - owns the validator API and implementation seam.
- `src/splits.rs` - owns the splits API and implementation seam.
- `src/generator.rs` - owns the generator API and implementation seam.
...
tier = "MVK required"
owner = "Data and cognition teams"
architectural_dependencies = ["alani-protocol", "alani-config", "alani-policy"]
aliases = []
```
...
1. Instantiate the template from `repo-templates/individual/alani-corpus`.
...
- `alani-corpus` builds as an independent Rust 2021 crate once a Rust toolchain is installed.
