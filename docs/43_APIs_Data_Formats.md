...
|---|---|
| Project | Alani |
| Document ID | ALANI-SPEC-43 |
| Status | Draft implementation specification |
| Version | 0.1.0-draft |
| Generated | 2026-04-30T21:27:04Z |
| Audience | Developers, data teams |
| Depends on | Doc 8, Doc 14, Doc 17 |

## Purpose

Shared schemas for ABI messages, configuration, events, and storage.

Shared APIs and data formats define how configuration, syscall envelopes, audit records, corpus examples, and device messages are serialized.
...
- ALANI-D43-REQ-01: The document MUST cover: JSON schema, binary ABI, message envelopes, configuration TOML, audit JSONL.
- ALANI-D43-REQ-02: Procedures MUST be reproducible from a clean checkout or clean artifact bundle.
- ALANI-D43-REQ-03: Inputs, outputs, owners, and approval gates MUST be named.
- ALANI-D43-REQ-04: Security and audit implications MUST be considered before release.
...
```json
{
  "schema_version": "alani.event.v1",
  "event_id": "evt_000001",
  "trace_id": "trc_000001",
  "component": "kernel.syscall",
  "operation": "sys_infer",
  "principal": "task:runtime-init",
  "resource": "model:mock-default",
  "decision": "allow",
  "status": "ok",
  "redaction": { "payload": "omitted", "reason": "sensitive" }
}
```

## Acceptance Criteria

- ALANI-D43-AC-01: A contributor can perform the procedure without private context.
- ALANI-D43-AC-02: The document names failure modes and how to recover.
- ALANI-D43-AC-03: The procedure can be mapped to CI or release evidence.
...
- RISK-D43-01: Manual process steps can diverge from automation.
- RISK-D43-02: Legal and data-source assumptions require human review before external publication.
...
## Expanded Shared Format Ownership
The following ownership rules prevent schema drift:

- `alani-abi` owns syscall and binary ABI data structures.
- `alani-protocol` owns transport and message schemas.
- `alani-config` owns configuration document schemas.
- `alani-corpus` owns corpus record schemas and split manifests.
- `alani-models` owns model manifests and adapter metadata.
- `alani-release` owns release manifests, SBOM metadata, checksums, and signing evidence.
