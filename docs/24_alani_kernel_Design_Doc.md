# Document 24: alani-kernel: Design Doc
...
|---|---|
| Project | Alani |
| Document ID | ALANI-SPEC-24 |
| Status | Draft implementation specification |
| Version | 0.1.0-draft |
| Generated | 2026-04-30T21:27:04Z |
| Audience | Kernel developers |
| Depends on | Doc 6, Doc 7, Doc 8, Doc 9, Doc 10, Doc 11, Doc 12, Doc 13, Doc 14 |

## Purpose

Detailed kernel repository design mapped to kernel specs.

This design document for alani-kernel provides implementation-ready guidance for Rust developers. Map the kernel repository to architectural modules and safety invariants.
...
- Primary aspects: boot sequence, module ownership, interrupt policy, unsafe review, QEMU path.
...
- alani-kernel owns its public crate boundary and README examples.
...
- ALANI-D24-REQ-01: alani-kernel MUST build independently where possible and MUST document sibling repository requirements.
- ALANI-D24-REQ-02: alani-kernel MUST expose only the APIs named in this document until an RFC expands them.
- ALANI-D24-REQ-03: Every public item MUST include rustdoc explaining safety, errors, and examples.
- ALANI-D24-REQ-04: Every repository MUST include README, Cargo.toml, src, tests, and CI workflow files.
...
- alani-kernel/Cargo.toml
- alani-kernel/src/lib.rs or src/main.rs
- alani-kernel/tests
- alani-kernel/.github/workflows/ci.yml
...
- ALANI-D24-AC-01: A developer can create alani-kernel from the provided template and understand its first implementation tasks.
- ALANI-D24-AC-02: README commands are copy-pasteable after repositories are placed as siblings.
- ALANI-D24-AC-03: Public APIs are traceable to an architecture or data-format document.
...
- RISK-D24-01: Repository drift can occur if shared interfaces are duplicated without tests.
- RISK-D24-02: Path dependencies in templates must be adjusted when repositories are published independently.
...
| Doc 10 | Must be read before implementing this document. |
| Doc 11 | Must be read before implementing this document. |
| Doc 12 | Must be read before implementing this document. |
| Doc 13 | Must be read before implementing this document. |
| Doc 14 | Must be read before implementing this document. |
